        ...
        public static List<Movie> GetAllMovies()
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConnectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
                    ...
                    while (reader.Read())
                    {
                        movies.Add(new Movie() {MovieID = reader.GetInt32(0), 
...
where MovieID = @movieID and SessionDate >= @startDate and SessionDate < @endDate";
        ...
        public static List<Session> GetAllSessionsInTheFuture()
        {
            ...
            {
                conn.Open();
                string sql = @"select SessionID, MovieID, SessionDate, CinemaNumber from [Session] where SessionDate > @startDate";
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            sessions.Add(new Session() {SessionID = reader.GetInt32(0), MovieID = reader.GetInt32(1), 
                                SessionDate = reader.GetDateTime(2), CinemaNumber = reader.GetByte(3) });
                        }
                    }
                }
            }
        ...
        public static Session GetSessionByID(int sessionID)
        {
            ...
            {
                conn.Open();
                string sql = @"select SessionID, MovieID, SessionDate, CinemaNumber from [Session] where SessionID = @sessionID";
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
